...
extension CommonFunction {
   ...
   /* func showMesaageBar(title: String? = nil,message : String? = nil,bstyle : BannerStyle){
    ...
    func isCodeSuccess(jsonResp : JSON) -> Bool{
        let code = APIResponseCode.code.init(fromRawValue: jsonResp[APIKeys.flag].intValue)
    ...
    func getCurrentLatLong() -> CLLocationCoordinate2D {
        ...
        if CLLocationManager.authorizationStatus() == .authorizedWhenInUse || CLLocationManager.authorizationStatus() == .authorizedAlways {
            curLocation = locManager.location
        }
        
        return curLocation.coordinate
    }
}
