...
extension AppDelegate : UNUserNotificationCenterDelegate{
    ...
    func handleAppViewControllerFlow(){
        if USERDEFAULTS.loadCustomObject(UDKeys.UserProfile) != nil{
            ...
            let storyboardID = User.loadLoggedInUser().isSeller! ? StoryboardId.SELLER : StoryboardId.USER
            let homeStoryboard = UIStoryboard(name: storyboardID, bundle: nil)
    ...
    func callUpdateDeviceToken(){
        ...
        let headerPrm : [String : String] = [
            APIKeys.empID       : objUser.customerId!,
        ]
        let param : [String : String] = [ APIKeys.deviceToken : USERDEFAULTS.loadCustomObject(UDKeys.DeviceToken) as! String ]
        
        WebServiceHelper.postWebServiceCall(WebAPI.BaseURL, params: param,headerPrm: headerPrm, isShowLoader: false, success: { (resJSON) in
    ...
    func callLogoutService(){
        ...
        WebServiceHelper.postWebServiceCall(WebAPI.BaseURL, params: nil,headerPrm: headerPrm, isShowLoader: false, success: { (resJSON) in
            if CommonFunction.Instance.isCodeSuccess(jsonResp: resJSON){
    ...
    func callToVersionCheckService(){
        ...
        WebServiceHelper.getWebServiceCall(WebAPI.BaseURL, params: nil, isShowLoader: false, success: { (resJSON) in
            if CommonFunction.Instance.isCodeSuccess(jsonResp: resJSON){
                ...
                self.window?.rootViewController!.showAlertWithCompletion(pTitle: "", pStrMessage: resJSON[APIKeys.message].stringValue, completionBlock: nil)
            }
        }) { (resError) in
            //ERROR
        }
